...
#
# OVERLAY FILE -- vllm/model_executor/models/qwen3_dflash.py
# Base: vLLM 0.27.1 as shipped in docker.io/stilldeadcode/vllm-radiance:0.9.3.
# Deviation from that base: TWO hunks, both marked "LOCAL PATCH 2026-08-28", both
# fixing the same failure -- upstream reads `qkv_proj.weight` unconditionally, and a
# packed int4 draft checkpoint (compressed-tensors weight_packed, or AutoGPTQ qweight)
# has no `.weight` attribute at all, so an int4 DFlash2 drafter raised AttributeError
# at load and could not be served. Both hunks route to the identity-matrix path that
# the fp8 drafter already used, which runs the layer's own quant kernel and is
# therefore exact for any quant method. Needed only for an INT4 draft checkpoint;
# an fp8 or bf16 draft runs on the stock file.
...
def _dflash_kv_weight_rows(qkv_proj, q_size: int) -> torch.Tensor:
    ...
    # LOCAL PATCH 2026-08-28 (radiance 0.9.3 / vLLM 0.27.1). Upstream reads
    # `qkv_proj.weight` unconditionally. A packed int4 draft (compressed-tensors
    # weight_packed, or AutoGPTQ qweight) has no `.weight` AT ALL, so this raised
    # `AttributeError: 'QKVParallelLinear' object has no attribute 'weight'` at load
    # and no int4 DFlash2 drafter could be served. Absent a dense float `.weight`,
    # take the identity-matrix path fp8 already uses: it runs the layer's own quant
    # kernel, so it is exact for any quant method (zero-points included) and needs
    # no unpack/dequant code of ours. Replaces the hand-written `_dense_kv_rows()`
    # unpacker our 0.26.0 overlay carried.
    ...
    device = (
        weight.device if weight is not None else next(qkv_proj.parameters()).device
    )
    eye = torch.eye(
...
class DFlashQwen3Model(nn.Module):
    ...
    def _build_context_kv_buffers(
        ...
        # LOCAL PATCH 2026-08-28: same AttributeError as above -- a packed int4
        # qkv_proj has no `.weight`. It needs the deferred path for the same reason
        # fp8 does: the quant method has not processed the weights yet.
        _w0 = getattr(layers_attn[0].qkv_proj, "weight", None)
        if _w0 is None or not _w0.is_floating_point() or _w0.dtype in _DFLASH_FP8:

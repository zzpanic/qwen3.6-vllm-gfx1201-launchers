...
# DEVIATIONS FROM MERGED UPSTREAM IN THIS FILE: deviation 3 applied — added the new
#   gumbel_noised_argmax() helper only. Did NOT apply the PR's refactor of
#   gumbel_block_argmax() to call gumbel_noised_argmax(); gumbel_block_argmax is
#   left byte-identical to 0.26.0's version (verified by diff), so the main
#   sampler path (MTP, eagle, etc.) is untouched.
# HAND-ADAPTED HUNKS (0.26.0 drift): none — the added hunk (gumbel_noised_argmax)
#   applied cleanly; the gumbel_block_argmax refactor hunk was deliberately
#   dropped per deviation 3, not because it failed to apply.
import torch

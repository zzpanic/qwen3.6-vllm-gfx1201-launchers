...
# DEVIATIONS FROM MERGED UPSTREAM IN THIS FILE: none in this file's own content.
#   NOTE: this class overrides draft_logits_spec() -> (torch.float32, -inf), and
#   that override IS LIVE here. CORRECTION 2026-08-22: an earlier version of this
#   header said it was dead code. That described the blunter first form of
#   deviation 1, in which the base class's draft_logits_spec() hook was not ported
#   at all. The hook WAS ported in the end -- see the base class at
#   v1/worker/gpu/spec_decode/speculator.py, which calls
#   `dtype, fill = self.draft_logits_spec(vllm_config)` at the draft_logits
#   allocation site -- so DFlash2's proposal cache is filled with -inf as upstream
#   intends, not 0.0. This matters: production runs
#   draft_sample_method=probabilistic, which is the only path that reads it.
# HAND-ADAPTED HUNKS (0.26.0 drift): none — new file, copied wholesale from merged
#   upstream. Confirmed commit fed7eea63395bbf75f5fd860d051f6d2afe5d961's fix
#   (load scores at tl.float64 if USE_FP64 else tl.float32, not always fp32) IS
#   present at the `scores = tl.load(...).to(tl.float64 if USE_FP64 else tl.float32)`
#   line in _selector_walk_kernel.

...
# DEVIATIONS FROM MERGED UPSTREAM IN THIS FILE: deviation 1 applied, in refined
#   form. The PR's two hunks here add a draft_logits_spec() hook and route the
#   draft_logits allocation through it. BOTH hunks are ported (the hook is needed
#   or DFlash2Speculator's override is dead code), but the BASE returns
#   `torch.float32` where merged upstream returns
#   `vllm_config.model_config.head_dtype`. float32 is exactly what stock 0.26.0
#   hardcoded at the allocation site, so every non-DFlash2 speculator -- MTP,
#   i.e. current production -- allocates a bit-identical tensor to stock. The
#   `fill` half is carried through unchanged, which is what makes
#   DFlash2Speculator.draft_logits_spec() -> (torch.float32, -inf) live.
#   Net effect vs 0.26.0: behaviour changes ONLY for a speculator that overrides
#   the hook, and only under draft_sample_method == "probabilistic".
# HAND-ADAPTED HUNKS (0.26.0 drift): none. Both PR hunks apply cleanly; only the
#   base return value differs, as described above.
...
class DraftModelSpeculator(BaseSpeculator):
    ...
    def draft_logits_spec(self, vllm_config: VllmConfig) -> tuple[torch.dtype, float]:
        ...
        LOCAL DEVIATION from merged upstream: upstream returns
        `vllm_config.model_config.head_dtype`; this returns `torch.float32`,
        which is what stock 0.26.0 hardcoded at the allocation site. That keeps
        the MTP path -- current production -- bit-identical to stock, which is
        the entire point of the deviation. The `fill` half is carried through
        unchanged so that DFlash2Speculator's override (float32, -inf) is live.
        """

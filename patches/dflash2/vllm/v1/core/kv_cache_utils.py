...
#
# LOCAL PATCH (2026-08-21). Complete file, based on stock vLLM 0.26.0
# `vllm/v1/core/kv_cache_utils.py`, with ONE change: in
# `resolve_kv_cache_block_sizes()`, the Mamba divisibility guard now reads
#
...
#
# NOT from upstream PR #52816 (DFlash2, merged 2026-08-21 as
# b389ac29465b33f9e9c534df221ea3c129e9793f) -- that PR does not touch this
# file. This fix originates from the third-party overlay at
# github.com/BMorgan1296/qwen3.6-vllm-gfx1201-launchers and is a hard boot
# requirement, not an optional tuning knob: without it, DFlash2 does not
# boot.
#
# CAVEAT ON THE EXACT FAILURE MODE -- read the control flow before repeating
# the claim. When the stock guard FIRES it `return`s early with
# (scheduler_block_size, scheduler_block_size), so the ValueError below is
# NOT reached on that path. The boot failure therefore surfaces downstream,
# where a hash_block_size of 1664 meets a group whose block size is 832
# (kv_cache_coordinator.py:84,477,564 and block_pool.py:487,488,551 all
# assert on that divisibility). The ValueError here is only reachable once
# the guard stops firing -- i.e. with this patch applied and a genuinely
# indivisible group. Either way the patch is what makes DFlash2 boot; only
# the error text differs. Phase 3 step 0 boots once WITHOUT the overlay to
# capture the real message rather than inherit this from a third party.
#
# Why: the DFlash2 draft declares `sliding_window: 2048` on all five layers,
# adding a sliding-window KV group (block_size 832) beside the existing
# 1664-token attention group. Under the stock guard, `hash_block_size`
# backs off to the scheduler LCM (1664) whenever a Mamba group's raw
# `block_size` differs from `cache_config.block_size` -- and it does here.
# The validation a few lines below then hard-fails at startup because 832
# does not divide 1664. With the fix, the guard instead keys off
# `mamba_cache_mode` directly (the condition this function's own docstring
# already describes it as checking -- the stock code just implements it as
# a block_size comparison instead). It no longer fires when
# `mamba_cache_mode == "align"`, so `hash_block_size` falls through to
# `math.gcd(*group_block_sizes)` = 832, and both the 1664 and 832 groups
# divide it cleanly.
#
# Side effect that is NOT DFlash2-specific: this file is mounted whenever
# `DFLASH2_PATCH=1`, independent of `SPEC`. Because MTP runs through the
# same `resolve_kv_cache_block_sizes()`, mounting this patch also moves
# MTP's own prefix-hash granularity from 1664 to 832 whenever mounted --
# which is why the MTP arm is re-benchmarked with the overlay mounted
# rather than read from pre-patch bench history. See
# patches/dflash2/README.md Phase 3 / Gate 3.
"""KV-Cache Utilities."""

...
# DEVIATIONS FROM MERGED UPSTREAM IN THIS FILE: deviation 2 applied — reverted commit
#   0a03f5ba1b38ab3dde1b6ddb064ca7bb7c09965a ("DFlash2: take the candidate top-k from
#   LogitsProcessor"), which moved this logic into logits_processor.py. The top-k
#   (_flashinfer_topk/_topk helpers) and the open-coded compute_candidates body
#   (lm_head.quant_method.apply + manual pad-mask + topk + TP all-gather +
#   output_multiplier/final_logit_softcapping) are restored inline, matching the
#   file's pre-0a03f5ba form. logits_processor.py itself is NOT ported.
# HAND-ADAPTED HUNKS (0.26.0 drift): none — this is a new file, copied from merged
#   upstream wholesale and then deviation 2 was reverted by hand.

...
# DEVIATIONS FROM MERGED UPSTREAM IN THIS FILE: none among the ported hunks, but
#   this file carries ONE ADDITION THAT IS NOT UPSTREAM AT ALL: _dense_kv_rows()
#   (below, ~line 344), taken from the third-party DFlash2 overlay at
#   github.com/BMorgan1296/qwen3.6-vllm-gfx1201-launchers and extended here with two
#   guards of ours. Merged upstream reads `a.qkv_proj.weight` directly and therefore
#   cannot load an int4 (packed compressed-tensors) draft at all. See that function's
#   docstring and patches/dflash2/README.md.
# HAND-ADAPTED HUNKS (0.26.0 drift): none — all 5 hunks (is_causal precedence in
#   _dflash_layer_causal, decoder_layer_cls/model_cls class attributes for DFlash2
#   subclassing) applied cleanly at their new offsets (0.26.0 is missing an
#   earlier, unrelated docstring change and some intervening code, which shifted
#   line numbers but not content).
...
def _dense_kv_rows(attn: nn.Module) -> torch.Tensor:
    """Return dense K/V rows from an unquantized or packed QKV projection.

    DFlash2-W4A16 checkpoints quantize the draft QKV projections. The fused
    context-KV precompute needs a dense matrix, while the regular draft forward
    continues to use the quantized projection.

    PORTED from the third-party DFlash2 overlay (not from PR #52816 -- merged
    upstream has no equivalent and therefore cannot load an int4 draft at all).
    See patches/dflash2/README.md. The two guards below are OURS;
    the overlay ships neither, and without them both failure modes are silent.
    """
    qkv = attn.qkv_proj
    weight = getattr(qkv, "weight", None)
    if weight is not None and weight.dim() == 2:
        # GUARD 1 (ours). A `float-quantized` (fp8 W8A8) checkpoint also stores a
        # 2D `weight`, but in float8_e4m3 with the scale held separately -- so this
        # fast path would return RAW UNSCALED fp8 rows and nothing would complain.
        # No fp8 arm is planned; fail loudly if one is ever tried.
        if weight.dtype in (torch.float8_e4m3fn, torch.float8_e4m3fnuz,
                            torch.float8_e5m2, torch.float8_e5m2fnuz):
            raise NotImplementedError(
                "DFlash context-KV precompute: qkv_proj.weight is float8 "
                f"({weight.dtype}); a float-quantized branch that applies "
                "weight_scale must be written first. Returning these rows "
                "unscaled would silently corrupt the draft."
            )
    ...
    if packed is None or scale is None:
        raise TypeError(
            "DFlash context-KV precompute needs qkv_proj.weight or packed "
            "weight_packed/weight_scale tensors."
        )

    # GUARD 2 (ours). The dequant below is scale-only: it carries NO zero-point
    # term. On an asymmetric checkpoint it is silently wrong -- the draft still
    # loads and still runs, it just proposes from a corrupted KV precompute, which
    # shows up only as unexplained low acceptance. The int4 checkpoint in use was
    # verified `symmetric: true` in Phase 2; this makes that a runtime invariant
    # rather than a note in a file.
    zero_point = getattr(qkv, "weight_zero_point", None)
    if zero_point is not None and zero_point.numel() and bool(zero_point.any()):
        raise NotImplementedError(
            "DFlash context-KV precompute: qkv_proj has a non-zero "
            "weight_zero_point (asymmetric quantization), but the dequant here "
            "has no zero-point term. This would corrupt the draft silently."
        )
    ...
    return dense.to(scale.dtype)[attn.q_size :]

...
| **L2 CPU (primary)** | `/dev/shm` region (host tmpfs, `--ipc=host`) | `KV_OFFLOAD` → GiB via `kvoff_resolve`, **clamped to both** the tmpfs cap (`statvfs`, minus 256 MiB margin) **and** the RAM cap (`MemAvailable − 3 GiB keep-free`); min 4 GiB, else off | **Pre-faulted (`MADV_POPULATE_WRITE`) and pinned (`cudaHostRegister`) — never swaps.** **24 GiB ≈ 762,000 tokens** at this boot, confirmed live by `kv_offload_tier_capacity_bytes{tier="cpu"}` = 25.76 GB (an earlier revision of this line said 16 GiB / ~508k; the tier was resized). Eviction: `KVOFF_POLICY` (default `lru`; `arc` available, scan-resistant). Admission: `KVOFF_STORE_THRESHOLD` (default 0 = admit on first sight; 2 = admit on second sighting). |
...
**The N=8 stride in numbers** (from the launcher): a Mamba group holds one recurrent state and the load path reads exactly one chunk, but the store path writes **27 MB per group per chunk** — ~70 snapshots for a long conversation where the GPU keeps 2. At N=8 the bytes per 8 chunks fall 72→30 (**0.417×**), so the tier holds proportionally more (**the specific pair "276,900 instead of 115,360" is retracted** — it came from a Mamba-storage share retracted twice since; at the measured 33,808 B/token the 24 GiB tier holds ~762,000 tokens, 3.3× the GPU cache). That is what keeps a conversation in RAM on the follow-up turn; a CPU hit costs 1–2 s vs the measured **64.26 s** fs→CPU promotion. Cost: prefix hits truncate down to an N-chunk (13,184-token) boundary.

...
Assembled by `make-dist.sh` on 2026-09-08T23:43:51Z from the working
tree at `<repo>`.

| Path | Source | What it is |
|---|---|---|
| `README.md` | kv-cache/ | **Start here.** Status, roadmap, architecture. |
| `docs/` | kv-cache/ | Handover, operations runbook, implementation, known issues, future work, references, and the experimental record. |
| `patches/` | kv-cache/ | The seven house patches, their apply order, and `patches/README.md` — what they need to run, which two are fatal on failure, and the env gate on each. **Reconcile against upstream before using.** |
